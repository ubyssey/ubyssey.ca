...
class AuthorPage(RoutablePageMixin, Page):
    ...
    def organize_media(self, media_type, request, context):
        ...
        if media_type == "photos":
            ...
            ).order_by(article_order + "updated_at")
        elif media_type == "visuals":
            # Get articles where this author is credited with something other than "author" and "org_role"
            authors_media = []
            keys = []
            for a in (
                ArticleAuthorsOrderable.objects.filter(
                    author=self, article_page__live=True
                )
                .exclude(Q(author_role="author") | Q(author_role="org_role"))
            ...
            authors_media = []
            keys = []
            for a in ArticleAuthorsOrderable.objects.filter(
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        if ArticleAuthorsOrderable.objects.filter(
            author=self, author_role="author"
        ).exists():
            media_types.append(("articles", "articles"))
        if (
            ArticleAuthorsOrderable.objects.filter(author=self)
            .exclude(author_role="author")
            .exists()
        ):
