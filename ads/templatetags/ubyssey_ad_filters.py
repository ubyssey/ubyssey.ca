...
def inject_ads(value, is_mobile):
    ...
    removeShortParagraphs = (
        lambda p: len(p.encode_contents()) >= ACCEPTABLE_PARAGRAPH_LENGTH
    )
    paragraphs = list(
        filter(removeShortParagraphs, soup.select("p:not(div > p, blockquote > p)"))
    )

    if PARAGRAPHS_PER_AD < len(
        paragraphs
    ):  # If the article is somehow too short for even one ad, it doesn't get any
        x = range(0, len(paragraphs), PARAGRAPHS_PER_AD)
        for n in x:
            if n > 0:  # Don't put an ad at the very beginning of the article
                if (
                    n // PARAGRAPHS_PER_AD
                ) > MAX_ADS:  # if we're above the max number of ads per article, stop!
                    break
                ...
                ad_string = render_to_string(
                    "ads/advertisement_inline.html", context=ad_context
                )
...
def specify_homepage_sidebar_ads(value, request):
    ...
    for div, orderable in list(zipped_placements_and_contents):
        ...
        if div:
            new_tag_soup = BeautifulSoup(
                render_to_string("ads/gpt_placement_tag.html", context=ad_context),
                "html5lib",
            )
            div.clear()
