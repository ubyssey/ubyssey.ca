...
class EventsTheme(object):
    ...
    def react(self, request):
        ...
        return render(
            request,
            "events/event_page_react.html",
            {"ical": ical, "rss": rss, "meta": meta},
        )
    ...
    def landing(self, request):
        ...
        week = {
            ...
            "days": [{"day": day.day, "day_of_week": day.strftime("%a"), "events": []}],
        }

        if request.GET.get("show_hidden"):
            if request.GET.get("category"):
                events = Event.objects.filter(
                    category=request.GET.get("category"), end_time__gte=day
                ).order_by("start_time")
            else:
            ...
            if request.GET.get("category"):
                ...
                events = Event.objects.filter(hidden=False, end_time__gte=day).order_by(
                    "start_time"
                )
        ...
        if request.GET.get("category"):
            ...
            ical = {
                "url": "https://ubyssey.ca/events/ical/?category="
                + request.GET.get("category"),
                "title": "Ubyssey's "
                + request.GET.get("category").capitalize()
                + " Around Campus iCal Feed",
            }

            rss = {
                "url": "https://ubyssey.ca/events/rss/?category="
                + request.GET.get("category"),
                "title": "Ubyssey's "
                + request.GET.get("category").capitalize()
                + " Around Campus rss Feed",
            }

            meta = {
                "title": request.GET.get("category").capitalize()
                + " Around Campus Calendar",
                "description": request.GET.get("category").capitalize()
                + " Around Campus collected by The Ubyssey",
                "url": "https://ubyssey.ca/events/?category="
                + request.GET.get("category"),
            }
        ...
        while len(calendar) < weekNum:
            ...
            elif day < timezone.now() - timedelta(
                hours=day.hour, minutes=day.minute, seconds=day.second
            ):
            ...
            if events_index >= len(events):
                ...
                while i < len(ongoing):
                    ...
                    if (
                        ongoing[i]
                        .end_time.astimezone(timezone.get_current_timezone())
                        .date()
                        == day.date()
                    ):
                        ongoing.pop(i)
                ...
                events[events_index].start_time = events[
                    events_index
                ].start_time.astimezone(timezone.get_current_timezone())
                events[events_index].end_time = events[
                    events_index
                ].end_time.astimezone(timezone.get_current_timezone())

                if (
                    request.GET.get("category") == "seminar"
                    or events[events_index].category != "seminar"
                ):
                    if (
                        closest_event == None
                        and timezone.now() <= events[events_index].start_time
                    ):
                ...
                if day > events[events_index].start_time:
                    ...
                    if day.date() == events[events_index].start_time.date():
                        ...
                        if (
                            not highlighted_feature in legend
                            and highlighted_feature != None
                        ):
                            legend.append(highlighted_feature)

                        week["days"][-1]["events"].append(events[events_index])
                        if (
                            not events[events_index].end_time.date()
        ...
        if event:
            ...
            event.start_time = event.start_time.astimezone(
...
async def update_events(request):
    ...
    async for event in Event.objects.filter(
        update_mode=1, end_time__gte=timezone.now()
    ):
        event.update_mode = 2
        await event.asave()
    ...
    wp_apis = [
        ...
        },
        {
            ...
            "categorize": {"default": "seminar", "community_type": [544, 546, 549]},
        },
        {
    ...
    ical_files = [
        ...
        },
        {
            ...
            "file": "https://thunderbirdarena.ubc.ca/?tribe-bar-date=2024-"
            + str("%02d" % datetime.now().month)
            + "-01&ical=1",
        ...
        },
        {
            "name": "Thunderbird Arena",
            "file": "https://thunderbirdarena.ubc.ca/?tribe-bar-date=2024-"
            + str("%02d" % ((datetime.now().month % 12) + 1))
            + "-01&ical=1",
            "create_function": Event.objects.ical_create_event,
            "instructions": {
                "category": "entertainment",
        ...
        },
        {
            ...
            "instructions": {
                "category": "community",
                "description_transform": lambda e: e.description.replace(
                    "UBC, UBC Vancouver, UBC students, UBC student life, UBC students, UBC events, events at UBC, UBC student events, UBC back to school, UBC back to school events, UBC campus, UBC campus events",
                    "",
                ),
        ...
        },
        {
            ...
            "instructions": {
                "category": "community",
                "hidden_override": lambda e: len(e.get("categories").cats)
                > 0,  # The scheduled events are all cringe but well categorized. Events added manually can be cool but categories typically aren't added. At some point we should be more sophistiacted in filtering this lmao
            },
        },
        {
            "name": "Green College",
            ...
            "instructions": {
                "category": "seminar",
                "description_transform": lambda e: e.description.split(
                    "in the series\n\n"
...
def create_ical(request):
    ...
    if request.GET.get("category"):
        cal["X-WR-CALNAME"] = (
            request.GET.get("category").capitalize() + " Around Campus from The Ubyssey"
        )
        cal["X-ORIGINAL-URL"] = (
            "https://ubyssey.ca/events/?category=" + request.GET.get("category")
        )
        cal["X-WR-CALDESC"] = (
            request.GET.get("category").capitalize()
            + " at UBC collected by The Ubyssey"
        )
        all_events = Event.objects.filter(
            hidden=False, category=request.GET.get("category")
        ).exclude(start_time=None, end_time=None)
    else:
        ...
        all_events = Event.objects.filter(hidden=False).exclude(
            category="seminar", start_time=None, end_time=None
        )
    ...
    for event in all_events:
        ...
        ical_event.add(
            "dtstart", event.start_time.astimezone(timezone.get_current_timezone())
        )
        ical_event.add(
            "dtend", event.end_time.astimezone(timezone.get_current_timezone())
        )
        if request.GET.get("category"):
            ical_event.add(
                "url",
                "https://ubyssey.ca/events/?category="
                + request.GET.get("category")
                + "&event="
                + event.hash,
            )
...
class EventsFeed(Feed):
    ...
    def description(self, category):
        if category:
            return (
                category.capitalize()
                + " around UBC campus collected by your friends at The Ubyssey"
            )
        else:
    ...
    def item_title(self, item):
        ...
        title = (
            item.start_time.strftime("%-m/%-d %-I:%M%P")
            + " "
    ...
    def get_context_data(self, item=None, **kwargs: Any) -> Dict[str, Any]:
        ...
        item.displayTime = format_event_date(
            item.start_time, item.end_time, weekDay=True
        )
