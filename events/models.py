...
class EventManager(models.Manager):
    def hashing(self, string):
        return base64.b64encode(
            hashlib.blake2b(string.encode(), digest_size=6).hexdigest().encode()
        ).decode()
    ...
    async def read_ical(self, f):
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                headers={"User-Agent": "The Ubyssey https://ubyssey.ca/"}
            ) as session:
                    ...
                    for component in cal.walk():
                        if component.name == "VEVENT":
                            if isinstance(component.decoded("dtstart"), datetime):
                                start = component.decoded("dtstart").astimezone(
                            ...
                            if timedelta(days=30) > timezone.now() - start:
                                if "instructions" in f:
                                    tasks.append(
                                        asyncio.create_task(
                                            create_function(
                                                component, f["name"], f["instructions"]
                                            )
                                        )
                                    )
                                else:
                                    tasks.append(
                                        asyncio.create_task(create_function(component))
                                    )
        ...
        try:
            if not await self.filter(event_url=ical_component.get("url")).aexists():
                event = await self.acreate(
                    ...
                    hash=self.hashing(
                        ical_component.get("summary")
                        + str(ical_component.decoded("dtstart"))
                    ),
                )
            else:
            ...
            if isinstance(ical_component.decoded("dtstart"), datetime):
                event.start_time = ical_component.decoded("dtstart").astimezone(
                    timezone.get_current_timezone()
                )
            else:
                event.start_time = datetime.combine(
                    ical_component.decoded("dtstart"),
                    time(),
                    tzinfo=timezone.get_current_timezone(),
                )

            if isinstance(ical_component.decoded("dtend"), datetime):
                event.end_time = ical_component.decoded("dtend").astimezone(
                    timezone.get_current_timezone()
                )
            else:
                event.end_time = datetime.combine(
                    ical_component.decoded("dtend"),
                    time(),
                    tzinfo=timezone.get_current_timezone(),
                )
            ...
            if "hidden_title_terms" in instructions:
                event.hidden = True in (
                    term in ical_component.get("summary")
                    for term in instructions["hidden_title_terms"]
                )
            ...
            print(
                "- " + ical_component.get("summary") + ": " + ical_component.get("url")
            )
            return None
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    api + "events/?order=desc&page=1&per_page=30"
                ) as response:
                    result = json.loads(await response.text())
                    # print("Connected to " + name)
                    ...
                    tasks = []
                    for i in result:
                        if timezone.now() < datetime.fromisoformat(i["end"]).astimezone(
                            timezone.get_current_timezone()
                        ):
                            tasks.append(
                                asyncio.create_task(
                                    self.wp_events_api_create_event(
                                        i, api, name, categorize, locations
                                    )
                                )
                            )
    ...
    async def wp_events_api_create_event(
        self, event_json, api, host, categorize, locations
    ):
        ...
        try:
            if not await self.filter(
                ...
                start_time=datetime.fromisoformat(event_json["start"]).astimezone(
                    timezone.get_current_timezone()
                ),
            ).aexists():
                event = await self.acreate(
            ...
            event.description = str(
                event_json["excerpt"]["rendered"].encode("utf-8"), "UTF-8"
            )

            event.start_time = datetime.fromisoformat(event_json["start"]).astimezone(
                timezone.get_current_timezone()
            )
            event.end_time = datetime.fromisoformat(event_json["end"]).astimezone(
                timezone.get_current_timezone()
            )
            ...
            if len(event_json["event-venues"]) > 0:
                if event_json["event-venues"][0] in locations:
                    ...
                    async with aiohttp.ClientSession(
                        ...
                        async with session.get(
                            api + "event-venues/" + str(event_json["event-venues"][0])
                        ) as response:
            ...
            req = Request(
                api, headers={"User-Agent": "The Ubyssey https://ubyssey.ca/"}
            )
            ...
            print(
                "- "
                + str(event_json["title"]["rendered"].encode("utf-8"), "UTF-8")
                + ": "
                + event_json["link"]
            )
            return None
    ...
    async def ubcevents_judge_hidden(self, event, ical):
        ...
        if (
            await self.filter(title=event.title, start_time=event.start_time)
            .exclude(event_url=event.event_url)
            .aexists()
        ):
            return True
        ...
        if "see description" in location:
            if (
                ...
                or "virtual" in description
                or "webinar" in description
            ):
                if (
                    not "hybrid" in location
                    and not "in-person" in location
                    and not "in-person" in description
                    and not "hybrid" in description
                ):
                    return True
        ...
        for i in [
            "coffee hour",
            "advanced research computing summer school",
            "Student Indoor Plant Sale at UBC Botanical Garden",
            "Grapes to Glass",
        ]:
        ...
        if "audience" in categories:
            if (
                "students" not in categories
                and "audience – community" not in categories
            ):
                return True
    ...
    async def gothunderbirds_create_event(self, ical_component):
        if not await self.filter(
            event_url=ical_component.get("url").replace("&amp;", "&")
        ).aexists():
            event = await self.acreate(
                title=ical_component.get("summary"),
            ...
            if event.update_mode != 2 and event.end_time.astimezone(
                timezone.get_current_timezone()
            ) <= timezone.now() - timedelta(
                days=7
            ):  # Go Thunderbirds is unqiue because the events are updated after they pass to include the result
                return None

        if event.hash == "":
            event.hash = self.hashing(
                ical_component.get("summary") + str(ical_component.decoded("dtstart"))
            )
        ...
        if "Men's" in ical_component.get("summary"):
            ...
            g = "W. "
        event.title = g + ical_component.get("summary").replace("UBC ", "").replace(
            "vs", "<br>UBC vs"
        ...
        if len(splitDesc) > 0:
            ...
            sport = (
                sport.replace("Men's ", "").replace("Women's ", "").replace("UBC ", "")
            )
            event.host = sport
        else:
            event.host = ""
        ...
        if isinstance(ical_component.decoded("dtend"), datetime):
            ...
            if (
                ical_component.decoded("dtend").day
                - ical_component.decoded("dtstart").day
                == 1
            ):
                event.end_time = datetime.combine(
                    ical_component.decoded("dtstart"),
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get("https://phas.ubc.ca/events") as response:
                    ...
                    tasks = []
                    for event in events:
                        start_time_str = event.find("span", class_="start").get_text(
                            strip=True
                        )
                        parsed_start_time = datetime.fromisoformat(start_time_str)
                        start_time = datetime.combine(
                            parsed_start_time.date(),
                    ...
                    print(
                        "UBC Physics and Astronomy finished "
                        + str(len(tasks))
                        + " tasks"
                    )
        except:
    ...
    async def phas_ubc_create_event(self, event_component):
        ...
        if not await self.filter(event_url=event_url).aexists():
            event = await self.acreate(
                ...
                hash=self.hashing(
                    title
                    + str(
                        event_component.find("span", class_="start").get_text(
                            strip=True
                        )
                    )
                ),
        ...
        if event.hash == "":
            event.hash = self.hashing(
                title
                + str(event_component.find("span", class_="start").get_text(strip=True))
            )

        # Extract start time
        start_time_str = event_component.find("span", class_="start").get_text(
            strip=True
        )
        parsed_start_time = datetime.fromisoformat(start_time_str)
        current_tz = timezone.get_current_timezone()
        start_time = datetime.combine(
            parsed_start_time.date(), parsed_start_time.time(), tzinfo=current_tz
        )

        # Extract location
        location_span = event_component.find("span", class_="location")
        location = (
            location_span.get_text(strip=True).replace("Event Location:", "").strip()
        )
        ...
        end_time = datetime.combine(
            parsed_end_time.date(), parsed_end_time.time(), tzinfo=current_tz
        )
    ...
    async def cs_ubc_create_event(self, ical_component):
        ...
        if ical_component.get("description", False):
            ...
            if "Location: " in ical_component.get("description"):
                s = ical_component.get("description")[
                    ical_component.get("description").index("Location: ")
                    + len("Location: ") :
                ]
    ...
    async def stats_ubc_create_event(self, ical_component):
        if timedelta(days=30) > abs(
            timezone.now()
            - ical_component.decoded("dtstart").astimezone(
                timezone.get_current_timezone()
            )
        ):
            if not await self.filter(event_url=ical_component.get("url")).aexists():
        ...
        while "Speaker" in description and "Description" in description:
            description = description.replace(
                description[
                    description.index("Speaker") : description.index("Description")
                    + len("Description")
                ],
                "",
            )
...
class Event(models.Model):
    ...
    hash = models.CharField(max_length=50, blank=False, null=True, default="")
    ...
    hidden = models.BooleanField(
        ...
        editable=True,
        help_text=(
            "Events that are only online, aren't for undergraduates, or don't have"
            " enough information to categorize should be automatically hidden. This"
            " means they don't show on the calendar."
        ),
    )
    ...
    update_mode = models.IntegerField(
        default=0,
        help_text=(
            "Make sure you select 'Manual input', otherwise this event will be"
            " overwritten or deleted during the calendar's automatic daily update"
        ),
    )
