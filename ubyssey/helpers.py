...
class ArticleHelper(object):
        ...
        articles = (
            Article.objects.annotate(
                age=ExpressionWrapper(
                    F("published_at") - timezone.now(), output_field=DurationField()
                ),
                reading=Case(
                    When(
                        ...
                        < datetime.strptime(
                            reading_times["morning_start"], timeformat
                        ).time()
                        else 0.0,
                    ),
                    When(
                        ...
                        if (
                            timezone.now().time()
                            >= datetime.strptime(
                                reading_times["midday_start"], timeformat
                            ).time()
                    ...
                    When(
                        ...
                        if timezone.now().time()
                        <= datetime.strptime(
                            reading_times["evening_start"], timeformat
                        ).time()
                        else 0.0,
                    ),
                    default=Value(0.5),
    ...
    def get_meta(article, default_image=None):
        ...
        return {
            ...
            if article.seo_description is not None
    ...
    def prepare_data(content):
        ...
        for chunk in content:
            if chunk["type"] == "code":
                ...
                gallery = ImageAttachment.objects.all().filter(
                    gallery__id=int(chunk["data"]["id"])
                )
                ...
                for index, image in enumerate(gallery):
                    if index % 2 == 0:
                        ...
                        result["code"][int(math.floor(index / 2))]["player"][
                            "image"
                        ] = {
                            "thumbnail": image.image.get_thumbnail_url(),
                            "medium": image.image.get_medium_url(),
