...
from ubyssey.events.sources import EventError, FacebookEvent, NoEventHandler, UBCEvent
...
def submit_landing(request):
    context = {
        "meta": {
            ...
            "description": (
                "Hosting an event on or off campus? Submit it to us and we'll feature"
                " it on our website!"
            ),
        }
    }
...
def submit_success(request):
    context = {
        "meta": {
            "title": "Submit an Event",
            "description": (
                "Thanks for your submission! Your event has been submitted for"
                " approval. We'll email you once it goes live on our site."
            ),
        }
    }
...
def submit_form(request):
    ...
    if request.POST.get("url_import") and event_url is not None:
        sources = {"calendar.events.ubc.ca": UBCEvent, "facebook.com": FacebookEvent}
...
def edit(request, secret_id):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            body = render_to_string(
                "events/email/edit.html",
                {"secret_id": event.secret_id, "title": event.title},
            )
    ...
    context = {"form": form, "meta": {"title": "Edit an Event"}}
...
def events(request):
    ...
    context = {
        "meta": {
            ...
            "description": (
                "Upcoming campus events from UBC clubs and student organizations"
            ),
            "url": reverse("events"),
        },
        "events": events,
