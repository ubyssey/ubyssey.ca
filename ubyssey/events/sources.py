...
ERROR_MESSAGE = (
    "Invalid url: The event could be private, or there could be an error in the url"
    ' itself. Check that the event is "public" and try again'
)
...
class FacebookEvent(object):
    ...
    def get_json(self):
        ...
        try:
            address = (
                json["place"]["location"]["street"]
                + ", "
                + json["place"]["location"]["city"]
            )
        except:
            address = None
...
class UBCEvent(object):
    ...
    def get_data(self):
        ...
        title = (
            soup.find("table", id="eventTitle")
            .find("h2", class_="bwStatusConfirmed")
            .find("a")
            .text
        )
    ...
    def get_date_groups(self, dates_string):
        ...
        re_date_and_time = re.search(
            ".*(\w{6,9}, \w{3,9} \d{1,2}, \d{4} \d{1}:\d{2} (?:AM|PM)).*",
            time_strings[1],
        )
