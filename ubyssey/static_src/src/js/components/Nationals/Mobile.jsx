...
class Mobile extends React.Component {
  constructor(props) {
    ...
    this.preId = "c-n-team-";
    this.state = {
      heights: this.props.teamData.map(() => {
        return "100%";
      }),
    };
  }
  ...
  componentDidMount() {
    let newHeights = this.state.heights.map((height, index) => {
      let temp = 0;
      for (const element of document.getElementById("c-n-team-" + index)
        .children) {
    ...
    return (
      ...
      >
        <div className="c-n-image-container">
          <div
            className="c-n-image"
            style={{ backgroundImage: 'url("' + team.image.thumbnail + '")' }}
          />
        </div>
  ...
  renderTeamContent(index, team, isSelected) {
    ...
    return (
      <div
        id={teamId}
        style={{ height: isSelected ? this.state.heights[index] : 0 }}
      >
        <TeamContent
          ref="teamContent"
          team={team}
          ...
          return (
            <div className="c-n-team-container">
              ...
              {this.renderTeamContent(
                index,
                team,
                selectedTeam ? selectedTeam.name === team.name : false,
              )}
            </div>
          );
        })}
