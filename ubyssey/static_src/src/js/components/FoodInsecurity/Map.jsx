...
class Map extends React.Component {
  ...
  zoomMap(e) {
    ...
    mX =
      (mX + ((this.state.scale - 1) * this.viewBox[2]) / this.state.scale) / 2;
    mY =
      (mY + ((this.state.scale - 1) * this.viewBox[3]) / this.state.scale) / 2;

    const newViewBox = [
      mX,
      mY,
      this.viewBox[2] / this.state.scale,
      this.viewBox[3] / this.state.scale,
    ];
    ...
    return (
      ...
      >
        ...
        <g id="Ocean">
          <rect
            className="cls-1"
            width={this.state.viewBox[2]}
            height={this.state.viewBox[3]}
          />
        </g>
        ...
        <g id="Locations">
          {Object.keys(locations).map((location) => {
            ...
            this.props.pointData.map((data) => {
              if (
                data.location.split(" ").join("").toLowerCase() === location
              ) {
  ...
  renderMobile(currentPoint) {
    const contentStyle = currentPoint
      ? { transform: "translateY(-350px)", opacity: 1 }
      : { transform: "translateY(0)" };
    return (
      <div className="c-i-map" height={this.MAP_HEIGHT}>
        <div
          className="c-i-map__image-container"
          style={{ backgroundColor: "#292b71" }}
        >
          {this.renderMap()}
          <div className="c-i-map__content-container" style={contentStyle}>
            <span>
              {" "}
              <i className="fa fa-arrow-down" onClick={() => this.reset()}>
                {" "}
                Back{" "}
              </i>{" "}
            </span>
            ...
            {currentPoint &&
              currentPoint.map((point) => {
                return (
                  <div className="c-i-map__content">
                    <div className="c-i-map__content-header">
                      <span className="c-i-map__content-title">
                        {point.name}
                      </span>
                    </div>
                    <div className="c-i-map__content-subtitle">
                      {point.location}
                    </div>
  ...
  renderDesktop(currentPoint) {
    const contentStyle = currentPoint
      ? { transform: "translate(0)", width: "350px", opacity: 1 }
      : { transform: "translate(-350px)", width: 0 };
    return (
      <div className="c-i-map" height={this.MAP_HEIGHT}>
        <div
          className="c-i-map__image-container"
        ...
        >
          {this.renderMap()}
          <div className="c-i-map__instructions">
            Double Click to Zoom | Click to Select
          </div>
        </div>
        <div className="c-i-map__content-container" style={contentStyle}>
          {currentPoint &&
          ...
          <i className="fa fa-arrow-left" onClick={() => this.reset()}>
            {" "}
            Back{" "}
          </i>
        </div>
    ...
    return (
      <div>
        ...
        {!this.props.isDesktop && (
          <div className="c-i-map__instructions">
            Double Tap to Zoom | Tap to Select{" "}
          </div>
        )}
      </div>
    );
