...
export default class Magazine extends Component {
  ...
  nav_clicked = (section) => {
    if (section === "System_failure") {
      if (!this.state.memoryLeak_right && this.state.systemFailure_right) {
        ...
        !this.state.systemFailure_right &&
        !this.state.memoryLeak_right &&
        !this.state.segFault_right
      ) {
  ...
  nav_position = (animation1, animation2, animation3, nav1, nav2, nav3) => {
    return (
      <ul className="nav">
        <li
          className={`${nav1} ${animation1}`}
          onClick={() => this.nav_clicked("Memory_leak")}
          style={{ color: "white" }}
        >
          {" "}
          <h3>MEMORY LEAK</h3>
        ...
        <li
          className={`${nav2} ${animation2}`}
          onClick={() => this.nav_clicked("Seg_fault")}
          style={{ color: "white" }}
        >
          {" "}
          <h3>SEG FAULT </h3>{" "}
        ...
        <li
          className={`${nav3} ${animation3}`}
          onClick={() => this.nav_clicked("System_failure")}
          style={{ color: "white" }}
        >
          {" "}
  ...
  nav_render = () => {
    ...
    if (this.state.section === "System_failure") {
      if (!this.state.systemFailure_right) {
        if (
          this.state.pre_memoryLeak_right &&
          this.state.pre_segFault_right &&
          this.state.pre_systemFailure_right
        ) {
          return this.nav_position(
            "animation_left1",
            "animation_left2",
  ...
  render() {
    ...
    return this.state.isloading ? (
      ...
      <div>
        <h1 className="mag_title" style={{ color: "white", fontSize: "70px" }}>
          SYSTEM REBOOT REQUIRED.{" "}
        </h1>

        <div className="horizontal_line"></div>

        <button className="menu_button" onClick={() => this.menu_clicked()}>
          Menu
        </button>

        <SideBar
          click={this.state.menu_clicked}
          menu_clicked={this.menu_clicked}
          dropdown_clicked={this.dropdown_clicked}
        />

        {nav}

        {this.state.section_display === "Editorial" && <Editorial />}
        {this.state.section_display === "Memory_leak" && (
          <MemoryLeak
            title={"Memory Leak"}
            articles={this.props.articles["Memory-Leak"]}
          />
        )}
        {this.state.section_display === "Seg_fault" && (
          <SegFault
            title={"Seg Fault"}
            articles={this.props.articles["Seg-Fault"]}
          />
        )}
        {this.state.section_display === "System_failure" && (
          <SystemFailure
            title={"System Failure"}
            articles={this.props.articles["System-Failure"]}
          />
        )}

        <footer className="footer">
          <a style={{ color: "white" }} href="https://www.ubyssey.ca/">
            {" "}
            BACK TO UBYSSEY.CA
          </a>
        </footer>
