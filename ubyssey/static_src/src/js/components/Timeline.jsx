...
class Timeline extends React.Component {
  ...
  prepareHeadline(headline) {
    if (headline) {
      return headline.length > 65
        ? headline.slice(0, 63).concat("...")
        : headline;
    }
    return "";
  }
  ...
  prepareUrl(slug) {
    return (
      ...
      window.location.pathname.replace(
        this.state.nodes[this.state.selectedNodeIndex].slug,
        slug,
      )
    );
  }
  ...
  renderButton(slug) {
    const mobileStyle = this.state.isMobile
      ? { left: "-35px" }
      : { alignSelf: "flex-end" };
    return (
      <div style={mobileStyle}>
        <a href={this.prepareUrl(slug)}>
          <div className="c-button c-button--small">Read More</div>
        </a>
      </div>
    );
  }
  ...
  renderDesktopNode(node, index) {
    ...
    const timelineNodeStyle =
      this.props.id === node.id
        ? "t-node-container t-node-selected"
        : "t-node-container";

    return (
      <div className={timelineNodeStyle}>
        <div ref="myRef" className="t-node">
          {this.props.id === node.id && <div className="t-node-solid"></div>}
        </div>
        <div className="t-node-hover">
          <div className="t-node-info">
            <div
              className="t-node-info-text"
              style={{
                right: index == this.state.nodes.length - 1 ? 0 : "auto",
              }}
            >
        ...
        <div className="t-node-date" style={dateStyle}>
          {date.toDateString().slice(4).replace(" 0", " ")}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={timelineNodeStyle}>
        <div className="t-node-mobile-box">
          ...
          <div className="t-node-info">
            <h3 className="o-headline">
              {this.prepareHeadline(node.headline)}
            </h3>
          </div>
        </div>
        <div className="t-node-mobile-box">
        ...
        <div className="t-node-mobile-box">{this.renderButton(node.slug)}</div>
      </div>
    );
  }
        ...
        {this.state.loaded && (
          <div className="t-container" style={mobileStyle}>
            <div
              className="t-title"
              onClick={() => {
                this.mobileHandle(window.innerWidth);
              }}
            >
              {this.state.isMobile && (
                <i
                  className="fa fa-bars"
                  style={{ fontSize: "20px", padding: "0 25px" }}
                ></i>
              )}
              <h1 className="o-headline">
                {this.props.title} {this.state.isMobile && "Timeline"}
              </h1>
            </div>
