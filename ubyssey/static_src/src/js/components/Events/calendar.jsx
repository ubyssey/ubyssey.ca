...
function useIsMobile() {
  const [isMobile, setIsMobile] = useState(
    window.innerWidth <= BP_DESKTOP_SIZE,
  );
        ...
        <div class="events-calendar">
          <header class="events">
            <div class="u-container">
              <div class="logo-area">
                <a
                  class="home-link"
                  href="/"
                  title="Go to The Ubyssey Homepage"
                >
                  <div
                    class="top-logo ubyssey_small_logo light-logo"
                    style={{
                      "background-image":
                        "url('https://ubyssey.ca/static/ubyssey/images/logos/ubyssey-logo-blue-light.e935f233a50c.svg')",
                    }}
                    alt="Ubyssey Logo"
                  ></div>
                  <div
                    class="top-logo ubyssey_small_logo dark-logo"
                    style={{
                      "background-image":
                        "url('https://ubyssey.ca/static/ubyssey/images/logos/ubyssey-logo-blue-dark.f3b3c0235809.svg')",
                    }}
                    alt="Ubyssey Logo"
                  ></div>
                </a>
            ...
            <EventsOptions
              ...
              setIsMobile={setIsDarkMode}
              getInitialStartDate={startDate}
              handleMonthNavigation={handleMonthNavigation}
              setIsMonthToggled={setIsMonthToggled}
              isMonthToggled={isMonthToggled}
              setStart={setStart}
            />
          </header>
...
function EventsCalendar({events, start, setStart, numberOfWeeks, setNumberOfWeeks, isDarkMode, setIsDarkMode, getInitialStartDate, handleMonthNavigation, setIsMonthToggled, isMonthToggled, isLoading}) {
    ...
    function placeEvents(calendar, event) {
      ...
      if (
        event.end_time.getTime() - event.start_time.getTime() >= d - h ||
        event.start_time.getHours() == 0
      ) {
        event.displayTime = "";
      }

      while (
        cur < new Date(event.end_time) ||
        cur.valueOf() == event.start_time.valueOf()
      ) {
        var delta = Math.floor((cur.getTime() - start.getTime()) / d);
        if (
          getDateString(new Date(start.getTime() + d * delta)) !=
          getDateString(cur)
        ) {
          delta = delta + 1;
        }
        if (delta >= 0 && delta < 7 * numberOfWeeks) {
          calendar[Math.floor(delta / 7)]["days"][delta % 7]["events"].push(
            event,
          );
        }
  ...
  function colourIn(legend) {
    ...
    for (let i = 0; i < legend.length; i++) {
      let r =
        200 + Math.floor(50 * Math.cos((i / legend.length) * 2 * Math.PI));
      let g =
        200 + Math.floor(50 * Math.sin((i / legend.length) * 2 * Math.PI));
      let b =
        200 +
        Math.floor(50 * Math.cos((i / legend.length) * 2 * Math.PI + Math.PI));
  ...
  var displayedEvents = events.filter(
          ...
          {legend.map((key, i) => (
            <li key={i} className={"legend-item " + slugify(key)}>
              <button
                ...
                onClick={(e) => {
                  console.log(e);
                  toggleCategory(e.target, searchParams, setSearchParams);
                }}
                title={key}
    ...
    for (let i = 0; i < events.length; i++) {
      if (events[i].hash == eventHash) {
        ...
        document.getElementsByTagName("title")[0].innerHTML =
          event.title.replace("<br>", "- ") + " - Ubyssey Events Around Campus";
        break;
      }
    }
  }
          ...
          {widthMode ? (
            <>
              <dialog id="event-dialog" open="" aria-modal="true">
                <div
                  className="events-info-shadow"
                  onClick={() => exitEvent(searchParams, setSearchParams)}
                ></div>
                <button
                  onClick={() => exitEvent(searchParams, setSearchParams)}
                >
  ...
  return (
    <div class="events-info">
      ...
      <div class={"events-info--content " + eventsTags(event)}>
        ...
        <p
          dangerouslySetInnerHTML={{
            __html:
              (event.host != null
                ? "<b>" +
                  (event.description ? event.host : "From " + event.host) +
                  "</b> "
        ...
        <p>
          <a
            href={event.event_url.replace("__AND__", "&")}
            target="blank"
            id="source_link"
          >
            {shortenUrl(event.event_url)}
          </a>
          {document.getElementById("calendar").getAttribute("authenticated") ==
            "True" && (
            <a
              href={"/admin/snippets/events/event/edit/" + event.id}
              id="edit_link"
            >
              edit
            </a>
