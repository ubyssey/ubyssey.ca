...
function openFeed() {
  document
    ...
    .classList.remove("home_infinitefeed_cutoff");
...
function closeFeed() {
  ...
  document
    .getElementById("feed-section")
    .classList.add("home_infinitefeed_cutoff");
...
updateTimeBox = setInterval(function () {
  ...
  if (
    ...
    document.getElementById("feed-section").offsetTop
  ) {
    var scroll =
      document.documentElement.scrollTop -
      document.getElementById("feed-section").offsetTop;
      ...
      if (article.classList.contains("article--infinitefeed")) {
        if (article.offsetTop > scroll && article.offsetTop < scroll + 500) {
          document.getElementById("timeBox").innerHTML =
            "<strong>" + article.getAttribute("time") + "</strong>";
          break;
        }
      }
    }
    ...
    if (document.documentElement.scrollTop < 100) {
      if (loader.hasAttribute("end") == false) {
        if (
          document
            .getElementById("feed-section")
            .classList.contains("home_infinitefeed_cutoff") == false
        ) {
          closeFeed();
        }
      }
