...
// Deals with YJS shared values https://docs.yjs.dev/getting-started/working-with-shared-types

// Converts YJS to JS values
export function sharedValueToJS(value) {
  if (value instanceof Y.Text) return value.toString();
  if (value instanceof Y.Array) return value.toArray().map(sharedValueToJS);
...
// TODO, potentially move to collaboration.js, also check if this is still being used at all?
export function mapCursorThroughTextReplacement(previous, next, position) {
  let prefix = 0;
  while (prefix < previous.length && prefix < next.length && previous[prefix] === next[prefix]) prefix += 1;
  let suffix = 0;
