// Synchronizes manuscript metadata between the form and Yjs

import * as Y from "yjs";

import { authorRows, newSharedAuthors, updateSharedAuthors } from "./authors.js";
import { fieldGroups, fieldValue, isCollaborativeText, setFieldValue } from "./fields.js";
import {
  mapCursorThroughTextReplacement,
  newSharedText,
  sharedValueToJS,
  updateSharedText,
} from "../../core/collaboration/shared_values.js";
import { AUTHORS_CHANGED_EVENT, AUTHORS_UPDATED_EVENT, FIELD_APPLIED_EVENT } from "./events.js";

const AUTHORS_KEY = "articleAuthors";
const FIELD_PREFIX = "field:";
const FIELD_SELECTOR = "[data-collaborative-metadata] [name]";

function sameValue(left, right) {
  return JSON.stringify(sharedValueToJS(left)) === JSON.stringify(sharedValueToJS(right));
}
...
export function setupMetadataCollaboration(form, metadata) {
  ...
  const applyField = (name, value, { remote = false } = {}) => {
    ...
    const selectionStart = preserveSelection ? mapCursorThroughTextReplacement(previousValue, nextValue, first.selectionStart) : null;
    const selectionEnd = preserveSelection ? mapCursorThroughTextReplacement(previousValue, nextValue, first.selectionEnd) : null;
  ...
  const applyAuthors = (authors) => {
    const rows = sharedValueToJS(authors);
