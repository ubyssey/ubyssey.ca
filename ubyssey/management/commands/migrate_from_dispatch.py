...
from article.models import (
    ...
    ArticleFeaturedMediaOrderable,
    ArticlePage,
    SpecialArticleLikePage,
)
from authors.models import AllAuthorsPage, AuthorPage
from home.models import HomePage
from images.models import GalleryOrderable, GallerySnippet
from images.models import UbysseyImage as CustomImage
from section.models import CategoryAuthor, CategorySnippet, SectionPage
from specialfeaturelanding.models import SpecialLandingPage
from videos.models import VideoAuthorsOrderable, VideoSnippet
...
def _home_page_init():
    ...
    try:
        ...
        root_page = Page.objects.get(
            slug="root"
        )  # Should always succeed in a wagtail installation, or else we have a bigger problem...
...
def _migrate_all_sections():
    ...
    for dispatch_section in dispatch_sections_qs:
        has_been_sent_to_wagtail = any(
            dispatch_section.slug == section_page.slug
            for section_page in wagtail_sections_qs
        )
...
def _migrate_all_authors():
    ...
    for person in dispatch_persons_qs:
        has_been_sent_to_wagtail = any(
            person.slug == wagtail_author.legacy_slug
            for wagtail_author in wagtail_authors_qs
        )
        if not has_been_sent_to_wagtail:
            print(
                "Sending author #" + str(person.pk) + " " + person.slug + " to wagtail"
            )
            wagtail_author = AuthorPage()

            wagtail_author.legacy_slug = (
                person.slug
            )  # there are a lot of invalid slugs in the original db

            wagtail_author.full_name = person.full_name
            wagtail_author.title = person.full_name

            # set slug
            existing_author_qs = AuthorPage.objects.filter(
            ...
            if http_res.status_code == 200:
                ...
                image_file = ImageFile(
                    BytesIO(http_res.content), name=wagtail_author.title
                )  # TODO FIX
                wagtail_image = CustomImage(title=wagtail_author.title, file=image_file)
                wagtail_image.legacy_filename = str(person.image)
                wagtail_image.save()
                if any(
                    collection.name == "Author Pics"
                    for collection in Collection.objects.all()
                ):
                    wagtail_image.collection = Collection.objects.get(
                        name="Author Pics"
                    )
                    wagtail_image.save()
...
def _migrate_all_categories():
    ...
    for dispatch_subsection in dispatch_subsections_qs:
        has_been_sent_to_wagtail = any(
            dispatch_subsection.slug == wagtail_category.slug
            for wagtail_category in wagtail_category_qs
        )
        if not has_been_sent_to_wagtail:
            print(
                "Sending category #"
                + str(dispatch_subsection.pk)
                + " "
                + dispatch_subsection.slug
                + " to wagtail"
            )
            ...
            wagtail_category.section_page = SectionPage.objects.get(
                slug=dispatch_subsection.section.slug
            )
            ...
            for author_obj in dispatch_subsection.authors.all():
                try:
                    ...
                    print(
                        "Getting author from Wagtail table for categories: "
                        + author_obj.person.full_name
                    )
                    category_author.author = AuthorPage.objects.get(
                        full_name=author_obj.person.full_name
                    )
                    ...
                    print(e)
                    error_string = (
                        author_obj.person.full_name + " is not a valid AuthorPage!"
                    )
                    print(error_string)
...
def _migrate_all_images():
    ...
    for old_image in old_images:
        has_been_sent_to_wagtail = any(
            str(old_image.img) == wagtail_image.legacy_filename
            for wagtail_image in wagtail_images
        )
        ...
        if has_been_sent_to_wagtail:
            ...
            print(
                ...
                + str(old_image.pk)
                + " url: "
                + str(old_image.get_absolute_url())
                + " to wagtail"
            )
            ...
            if http_res.status_code != 200:
                print(
                    ...
                    + str(old_image.get_absolute_url())
                    + " recieved "
                    + str(http_res.status_code)
                )
                tag_for_error = True
                http_res = requests.get(
                    "https://www.ubyssey.ca/static/ubyssey/images/ubyssey-logo-square.7fdeb5ac7f29.png"
                )
            wagtail_image_title = (  # should never actually be used, but just in case
                "default_title"
            )
            ...
            image_file = ImageFile(
                BytesIO(http_res.content), name=str(old_image.img)[15:]
            )  # old filename includes directory crap. Slice is to get rid of that
            ...
            if any(
                collection.name == str(old_image.img)[7:14]
                for collection in Collection.objects.all()
            ):
                wagtail_image.collection = Collection.objects.get(
                    name=str(old_image.img)[7:14]
                )
                wagtail_image.save()
            else:
            ...
            if len(old_image.authors.all()) > 0:
                ...
                try:
                    wagtail_image.author = AuthorPage.objects.get(
                        legacy_slug=old_author.person.slug
                    )
                    wagtail_image.legacy_authors = old_image.get_author_string()
                    wagtail_image.save()
                except:
                    print(
                        "Couldn't find an author with the slug "
                        + old_author.person.slug
                    )
...
def _migrate_all_image_galleries():
    ...
    for old_gallery in old_galleries:
        has_been_sent_to_wagtail = any(
            old_gallery.pk == wagtail_gallery.legacy_pk
            for wagtail_gallery in wagtail_galleries
        )
        if not has_been_sent_to_wagtail:
            print(
                "Sending gallery #"
            ...
            for image_attachment_object in old_gallery.images.all():
                ...
                if image_attachment_object.image:
                    ...
                    gallery_orderable.image = CustomImage.objects.get(
                        legacy_pk=image_attachment_object.image.pk
                    )
...
def _migrate_all_videos():
    ...
    for old_video in old_videos:
        has_been_sent_to_wagtail = any(
            old_video.url == wagtail_video.url for wagtail_video in wagtail_videos
        )
        if not has_been_sent_to_wagtail:
            print(
                "Sending video #"
            ...
            for dispatch_author in old_video.authors.all():
                ...
                if AuthorPage.objects.get(legacy_slug=dispatch_author.person.slug):
                    ...
                    has_author_already = any(
                        video_author.author.legacy_slug == dispatch_author.person.slug
                        for video_author in wagtail_video.video_authors.all()
                    )
                    if not has_author_already:
                        ...
                        wagtail_author_orderable.author = AuthorPage.objects.get(
                            legacy_slug=dispatch_author.person.slug
                        )
...
def _migrate_all_pages():
    ...
    for head_page in dispatch_head_pages_qs:
        ...
        dispatch_page_qs = dispatch_models.Page.objects.filter(
            slug=current_slug
        ).order_by("revision_id")
        for dispatch_page_revision in dispatch_page_qs:
            print(
                "Sending article revision "
                + str(dispatch_page_revision)
                + " to wagtail"
            )
            # first check if there's an article with this slug already:
            wagtail_special_article_qs = SpecialArticleLikePage.objects.filter(
                slug=current_slug
            )
            ...
            if len(wagtail_special_article_qs) < 1:
                ...
                print(
                    "Sending article revision PK "
                    + str(dispatch_page_revision.pk)
                    + " "
                    + str(dispatch_page_revision.slug)
                    + " to wagtail"
                )
                wagtail_special_article = SpecialArticleLikePage()
                wagtail_special_article.created_at_time = (
                    dispatch_page_revision.created_at
                )
                wagtail_special_article.slug = dispatch_page_revision.slug
                wagtail_special_article.title = dispatch_page_revision.title
                wagtail_section = SectionPage.objects.get(
                    slug="pages"
                )  # note - assumes 'pages' section exists
                wagtail_section.add_child(instance=wagtail_special_article)
            else:
                wagtail_special_article = wagtail_special_article_qs.get(
                    slug=current_slug
                )

            if (
                dispatch_page_revision.revision_id
                > wagtail_special_article.legacy_revision_number
            ):
                wagtail_special_article.legacy_revision_number = (
                    dispatch_page_revision.revision_id
                )
                ...
                if AuthorPage.objects.get(legacy_slug="ubyssey-staff"):
                    ...
                    # Otherwise we'll just get a bunch of redundant orderables
                    has_author_already = any(
                        article_author.author.legacy_slug == "ubyssey-staff"
                        for article_author in wagtail_special_article.article_authors.all()
                    )
                    if not has_author_already:
                        ...
                        wagtail_author_orderable.author = AuthorPage.objects.get(
                            legacy_slug="ubyssey-staff"
                        )
                        wagtail_author_orderable.save()
                # SEO stuff
                if dispatch_page_revision.seo_keyword:
                    wagtail_special_article.seo_keyword = (
                        dispatch_page_revision.seo_keyword
                    )
                if dispatch_page_revision.seo_description:
                    wagtail_special_article.seo_description = (
                        dispatch_page_revision.seo_description
                    )
                # "template"
                ...
                if dispatch_page_revision.featured_image:
                    if len(wagtail_special_article.featured_media.all()) < 1:
                        ...
                        if old_img_obj.image:
                            featured_media_orderable.image = CustomImage.objects.get(
                                legacy_pk=old_img_obj.image.pk
                            )
                            featured_media_orderable.save()
                    else:
                        # There is an image orderable ALREADY associated with this article
                        if any(
                            featured_media_orderable.image
                            for featured_media_orderable in wagtail_special_article.featured_media.all()
                        ):
                            old_img_obj = dispatch_page_revision.featured_image
                            featured_media_orderable = (
                                wagtail_special_article.featured_media.all()[0]
                            )
                            if old_img_obj.caption:
                                featured_media_orderable.caption = old_img_obj.caption
                            if old_img_obj.credit:
                            ...
                            if old_img_obj.image:
                                if (
                                    old_img_obj.image.pk
                                    != featured_media_orderable.image.legacy_pk
                                ):
                                    try:
                                        featured_media_orderable.image = (
                                            CustomImage.objects.get(
                                                legacy_pk=old_img_obj.image.pk
                                            )
                                        )
                                    except Exception as e:
                                        print(
                                            "No image found corresponding to "
                                            + str(old_img_obj.image.pk)
                                        )
                            featured_media_orderable.save()
                ...
                try:
                    for node in dispatch_page_revision.content:
                        ...
                        if node_type == "paragraph":
                            ...
                            try:
                                old_image = dispatch_models.Image.objects.get(
                                    pk=node["data"]["image_id"]
                                )
                                new_image = CustomImage.objects.get(
                                    legacy_pk=old_image.pk
                                )
                                block_type = "image"
                                block_value = {}
                                block_value["image"] = new_image.pk
                                block_value["style"] = node["data"].get(
                                    "style", "default"
                                )
                                ...
                                block_type = "richtext"
                                block_value = (
                                    "<p>DISPATCH IMAGE EMBED ERROR WITH ARTICLE</p>"
                                )
                            except CustomImage.DoesNotExist as e:
                                print(e)
                                block_type = "richtext"
                                block_value = (
                                    "<p>WAGTAIL IMAGE EMBED ERROR WITH ARTICLE</p>"
                                )
                        elif node_type == "code":
                            block_type = "raw_html"
                            try:
                            ...
                            block_type = "video"
                            block_value = {}
                            block_value["embed"] = node["data"].get(
                                "caption", "https://www.youtube.com/watch?v=e_fTr5XHh9U"
                            )  # Look for this later when
                            ...
                            if node["data"].get("id", 0) != 0:
                                try:
                                    old_gallery = (
                                        dispatch_models.ImageGallery.objects.get(
                                            id=node["data"]["id"]
                                        )
                                    )
                                    block_value = slugify(old_gallery.title)[:48] + str(
                                        old_gallery.pk
                                    )
                            ...
                            block_type = "raw_html"
                            try:
                                block_value = embeds.WidgetEmbed.render(
                                    data=node["data"]
                                )
                        ...
                        # SQL says this never actually occurs
                            ...
                            try:
                                block_value = (
                                    node["data"]["svg"] + node["data"]["initScript"]
                                )
                            except:
                        ...
                        wagtail_streamfield_node["id"] = hashlib.sha1(
                            str(wagtail_streamfield_node).encode("utf-8")
                        ).hexdigest()
                ...
                wagtail_special_article.latest_revision_created_at = (
                    dispatch_page_revision.updated_at
                )
                ...
                if dispatch_page_revision.published_at:
                    ...
                    wagtail_special_article.first_published_at = (
                        dispatch_page_revision.published_at
                    )
                    wagtail_special_article.last_published_at = (
                        dispatch_page_revision.updated_at
                    )
                    wagtail_special_article.save()
...
def _migrate_all_articles():
    # dispatch_article
    dispatch_head_articles_qs = dispatch_models.Article.objects.filter(
        head=True
    ).filter(id__gte=101000)

    for head_article in dispatch_head_articles_qs:
        current_slug = head_article.slug

        dispatch_article_qs = dispatch_models.Article.objects.filter(
            slug=current_slug
        ).order_by("revision_id")
        ...
        for dispatch_article_revision in dispatch_article_qs:
            print(
                "Sending article revision "
                + str(dispatch_article_revision)
                + " to wagtail"
            )
            # first check if there's an article with this slug already:
            wagtail_article_qs = ArticlePage.objects.filter(slug=current_slug).exclude(
                url_path__istartswith="/ubyssey/guide/2021"
            )
            ...
            if len(wagtail_article_qs) < 1:
                ...
                print(
                    "Sending article revision PK "
                    + str(dispatch_article_revision.pk)
                    + " "
                    + str(dispatch_article_revision.slug)
                    + " to wagtail"
                )
                ...
                wagtail_section = SectionPage.objects.get(
                    slug=head_article.section.slug
                )
            ...
            if (
                dispatch_article_revision.revision_id
                > wagtail_article.legacy_revision_number
            ):
                ...
                for dispatch_author in dispatch_article_revision.authors.all():
                    ...
                    if AuthorPage.objects.get(legacy_slug=dispatch_author.person.slug):
                        ...
                        # Otherwise we'll just get a bunch of redundant orderables
                        has_author_already = any(
                            article_author.author.legacy_slug
                            == dispatch_author.person.slug
                            for article_author in wagtail_article.article_authors.all()
                        )
                        if not has_author_already:
                            wagtail_author_orderable = ArticleAuthorsOrderable()
                ...
                if dispatch_article_revision.seo_description:
                    wagtail_article.seo_description = (
                        dispatch_article_revision.seo_description
                    )
                # "template"
                ...
                if dispatch_article_revision.template_data:
                    ...
                    if "header_layout" in processed_template_data:
                        wagtail_article.header_layout = processed_template_data[
                            "header_layout"
                        ]
                    wagtail_article.legacy_template_data = (
                        dispatch_article_revision.template_data
                    )
                ...
                if dispatch_article_revision.is_breaking:
                    wagtail_article.is_breaking = bool(
                        dispatch_article_revision.is_breaking
                    )
                if dispatch_article_revision.breaking_timeout:
                    wagtail_article.breaking_timeout = (
                        dispatch_article_revision.breaking_timeout
                    )
                ...
                if dispatch_article_revision.featured_image:
                    if len(wagtail_article.featured_media.all()) < 1:
                        ...
                        if any(
                            featured_media_orderable.image
                            for featured_media_orderable in wagtail_article.featured_media.all()
                        ):
                            old_img_obj = dispatch_article_revision.featured_image
                            featured_media_orderable = (
                                wagtail_article.featured_media.all()[0]
                            )
                            if old_img_obj.caption:
                                featured_media_orderable.caption = old_img_obj.caption
                            if old_img_obj.credit:
                ...
                if dispatch_article_revision.featured_video:
                    if len(wagtail_article.featured_media.all()) < 1:
                        ...
                        if old_vid_obj.video:
                            try:
                                featured_media_orderable.video = (
                                    VideoSnippet.objects.get(url=old_vid_obj.video.url)
                                )
                            except exceptions.ObjectDoesNotExist as e:
                                print(e)
                        featured_media_orderable.save()
                        ...
                        if any(
                            ...
                            old_vid_obj = dispatch_article_revision.featured_video
                            featured_media_orderable = (
                                wagtail_article.featured_media.all()[-1]
                            )
                            if (
                                old_vid_obj.video.url
                                != featured_media_orderable.video.url
                            ):
                                featured_media_orderable.sort_order = 1
                                featured_media_orderable.article_page = wagtail_article
                                if old_vid_obj.caption:
                                    featured_media_orderable.caption = (
                                        old_img_obj.caption
                                    )
                                if old_vid_obj.credit:
                                    featured_media_orderable.credit = old_img_obj.credit
                                if old_vid_obj.video:
                                    try:
                                        featured_media_orderable.video = (
                                            VideoSnippet.objects.get(
                                                url=old_vid_obj.video.url
                                            )
                                        )
                                    except exceptions.ObjectDoesNotExist as e:
                                        print(e)
                ...
                if dispatch_article_revision.subsection:
                    wagtail_article.category = CategorySnippet.objects.get(
                        slug=dispatch_article_revision.subsection.slug
                    )
                ...
                log_entry_change.save()
                wagtail_article.latest_revision_created_at = (
                    dispatch_article_revision.updated_at
                )
                wagtail_article.save()
                # Ensure all Wagtail _publish_ timestamps correspond to their Dispatch counterparts
                if dispatch_article_revision.published_at:
                    wagtail_revision.publish()
                    wagtail_article.first_published_at = (
                        dispatch_article_revision.published_at
                    )
                    wagtail_article.last_published_at = (
                        dispatch_article_revision.updated_at
                    )
                    wagtail_article.explicit_published_at = (
                        wagtail_article.last_published_at
                    )
                    wagtail_article.save()
                    log_entry_publication = PageLogEntry.objects.all()[0]
                    log_entry_publication.timestamp = (
                        dispatch_article_revision.updated_at
                    )
                    log_entry_publication.save()
...
def _fix_article_pages():
    qs = ArticlePage.objects.filter(
        first_published_at__isnull=False, explicit_published_at__isnull=True
    )
    for article in qs:
        print(article.slug)
...
def _fix_guide_articles():
    ...
    guide_articles_qs = (
        guide2020.get_descendants().type(ArticlePage).specific()
    )  # important!
    ...
    for guide_article in guide_articles_qs:
        ...
        print(
            guide_article.slug
            + " publish date set to: "
            + str(guide_article.explicit_published_at)
        )
