...
class ArticleMixin(object):
    ...
    def get_frontpage_qs(
        self, sections=[], exclude=[], limit=7, is_published=True, max_days=14
    ):
        articles = (
    ...
    def get_frontpage_sections(self, exclude=None):
        ...
        for section in sections:
            articles = (
                Article.objects.exclude(id__in=exclude)
    ...
    def get_reading_list(self, article, ref=None, dur=None):
        ...
        return {"ids": ",".join([str(a.parent_id) for a in articles]), "name": name}
    ...
    def get_breaking_news(self):
        ...
        return Article.objects.filter(
            is_published=True, is_breaking=True, breaking_timeout__gte=timezone.now()
        )
...
class DispatchPublishableViewMixin(object):
    ...
    def get_queryset(self):
        ...
        if self.preview_id is not None and self.version is not None:
            ...
            return (
                super()
                .get_queryset()
                .filter(revision_id=self.version, preview_id=self.preview_id)
            )
        return super().get_queryset().filter(is_published=True)

    def render_to_response(self, context, **response_kwargs):
        """
        In design, adds to the view counter before rendering the page. We do this as late as possible to try to prevent adding to the view counter in the event of errors
        In practice this has been buggy. Possibly flag this for depreciation
        """
    ...
    def get_article_meta(self, default_image=None):
        ...
        meta["description"] = (
            ...
            if self.object.seo_description is not None
            else self.object.snippet
        )
...
class GuideViewMixin(object):
    ...
    def get_context_data(self, **kwargs):
        ...
        articles = (
            Article.objects.filter(
                is_published=True, section__slug="guide", tags__name=self.year
            )
            .order_by("published_at")
        ...
        if hasattr(self, "object"):
            ...
            try:
                next_a = articles.get(
                    slug=self.object.template_fields["next_a"], is_published=True
                )
            except:
                next_a = None
            try:
                next_b = articles.get(
                    slug=self.object.template_fields["next_b"], is_published=True
                )
            except:
                next_b = None
...
class SubsectionMixin(object):
    ...
    def get_subsections(self, section):
        article_query = Article.objects.filter(
            subsection_id=OuterRef("id"), is_published=True
        ).order_by(F("published_at").desc(nulls_last=True))
        subsection_query = Subsection.objects.annotate(
            published_at=Subquery(article_query.values("published_at")[:1])
        ).filter(is_active=True, section_id=section.id)
        return list(subsection_query)

    def get_featured_subsection_articles(self, subsection, featured_articles):
        featured_articles_ids = list(
            featured_articles.values_list("id", flat=True)[0:4]
        )
        return (
            subsection.get_published_articles().exclude(id__in=featured_articles_ids)[
                0:3
            ]
            if subsection.get_published_articles()
            .exclude(id__in=featured_articles_ids)
            .exists()
            else subsection.get_published_articles()[0:3]
        )
...
class SectionMixin(SubsectionMixin, object):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        try:
            ...
            self.featured_articles = (
                Article.objects.filter(is_published=True)
                .filter(Q(subsection_id=filter_id) | Q(section_id=filter_id))
                .order_by("-published_at")
            )
    ...
    def get_template_names(self):
        ...
        try:
            template_names += [
                "%s/%s" % (self.section.slug, self.default_template),
    ...
    def get_queryset(self):
        if self.order == "newest":
            ...
            order_by = "published_at"
        article_list = (
            super().get_queryset().filter(is_published=True).order_by(order_by)
        )
        if self.query:
...
class ArchiveListViewMixin(object):
    ...
    def __get_years(self):
        ...
        """
        publish_dates = Article.objects.filter(is_published=True).dates(
    ...
    def get_queryset(self):
        ...
        article_qs = (
            Article.objects.prefetch_related("authors", "authors__person")
            .select_related("section", "featured_image")
