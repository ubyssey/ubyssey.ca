...
from dispatch.models import Article, Author, Person, Subsection, Tag, Video
...
class UpcomingEventsWidget(Widget):
    ...
    def context(self, result):
        ...
        events = (
            Event.objects.filter(is_submission=False)
...
class FeaturedSubsectionWidget(Widget):
    ...
    def context(self, result):
        ...
        if result["featured_subsection"]:
            subsection = Subsection.objects.filter(is_active=True).filter(
                name__icontains=result["featured_subsection"]
            )[:1]
        ...
        articles = (
            ...
            .filter(subsection_id=subsection.id)
            .order_by("-published_at")[:num_articles]
        )
...
class FeaturedVideosWidget(Widget):
    ...
    def context(self, result):
        ...
        if len(tag) > 0:
            video_list = Video.objects.filter(tags=tag.get()).order_by("-created_at")[
                :num_videos
            ]
        ...
        if video_list:
            ...
            for index, video in enumerate(video_list):
                    ...
                    video_list[index].videoAuthors.append(
                        {
                            ...
                            "link": VideoHelper.get_media_author_url(person.slug),
                        }
                    )
                ...
                if match:
                    ...
                    FeaturedVideosWidget.logger.warning(
...
class WeeklyEventsWidget(Widget):
    ...
    def context(self, data):
        data["events"] = (
            data["events"].order_by("start_time").filter(is_published=True)[:5]
        )
        return data
...
class FrontPageDefault(Widget):
    ...
    sidebar = WidgetField(
        "First Widget on Sidebar",
        [
            UpcomingEventsWidget,
            TwitterFrontPage,
            FeaturedSubsectionWidget,
            FeaturedVideosWidget,
            AdvertisementWidget,
        ],
        required=True,
    )
    sidebar2 = WidgetField(
        "Second Widget on Sidebar",
        [
            UpcomingEventsWidget,
            TwitterFrontPage,
            FeaturedSubsectionWidget,
