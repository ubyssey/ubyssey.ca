...
from section.models import CategorySnippet, SectionablePage, SectionPage
from specialfeaturelanding.models import SpecialLandingPage
from videos.models import VideoSnippet, VideosPage


class SectionPageOrderables(Orderable):
    page = ParentalKey(
        "archive.ArchivePage", on_delete=models.CASCADE, related_name="sections_filters"
    )
...
class MagazineOrderables(Orderable):
    page = ParentalKey(
        ...
        related_name="magazines_filters",
    )
...
class SpoofOrderables(Orderable):
    page = ParentalKey(
        "archive.ArchivePage", on_delete=models.CASCADE, related_name="spoofs_filters"
    )
...
class ArchivePage(RoutablePageMixin, Page):
    ...
    content_panels = Page.content_panels + [
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    "List all the category snippets you want to have in the magazine"
                    " filter"
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    "List all the category snippets you want to have in the spoof"
                    " filter"
                ),
    ...
    def get_paginated_articles(self, context, objects, video_section, request):
        ...
        context[
            "page_obj"
        ] = paginated_articles  # this object is often called page_obj in Django docs. Careful, because but Page means something else in Wagtail
    ...
    def get_year_objects(self, objects, videos_section):
        if videos_section == False:
            ...
            return objects.filter(
                created_at__gte=str(self.year) + "-01-01",
                created_at__lte=str(self.year + 1) + "-12-31",
            )
    ...
    def get_archive_general_articles(self, request):
        ...
        if search_query:
            ...
            if len(articles) < 1 and len(videos) > 0:
                video_section = True
                context = self.get_paginated_articles(
                    context, videos, video_section, request
                )
                context["video_section"] = True
            else:
                context = self.get_paginated_articles(
                    context, articles, video_section, request
                )
        else:
            context = self.get_paginated_articles(
                context, articles, video_section, request
    ...
    def get_section_articles(self, request, sections_slug="magazine"):
        ...
        articles = (
            ArticlePage.objects.from_section(section_slug=sections_slug).live().public()
        )
    ...
    def get_magazine_articles(self, request, magazine_slug):
        ...
        if len(SpecialLandingPage.objects.filter(category__slug=magazine_slug)) > 0:
            articles = ArticlePage.objects.from_magazine_special_section(
                section_slug=magazine_slug
            )
        else:
            articles = (
                ArticlePage.objects.live().public().filter(category__slug=magazine_slug)
            )
    ...
    def get_spoof_articles(self, request, spoof_slug="All Spoofs"):
        ...
        if spoof_slug == "All Spoofs":
            ...
            for iter in self.spoofs_filters.all():
                sections = (
                    SectionPage.objects.filter(categories__slug=iter.spoof_filter.slug)
                    .live()
                    .public()
                )
                articles = (
                    articles
                    | ArticlePage.objects.from_section(section_slug=sections[0].slug)
                    .live()
                    .public()
                )
        else:
            sections = (
                SectionPage.objects.filter(categories__slug=spoof_slug).live().public()
            )
            if sections[0].title.capitalize() != "Humour":
                ...
                articles = (
                    ...
                    .public()
                    .filter(category__slug=spoof_slug)
                )
