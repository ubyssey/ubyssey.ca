...
class AudioBlock(blocks.StructBlock):
    ...
    audio = DocumentChooserBlock(
        required=True, help_text="File format: .m4a, .mp4, .mp, .wav, or .ogg"
    )
...
class PullQuoteBlock(blocks.StructBlock):
    ...
    audio = DocumentChooserBlock(
        required=False,
        help_text="Optional, file format: .m4a, .mp4, .mp, .wav, or .ogg",
    )
        ...
        blocks.StructBlock(
            [
                ...
                (
                    ...
                    blocks.CharBlock(
                        help_text=(
                            "Intended to be shared with a header so that this button"
                            " will send the user to the section of the page with said"
                            " header"
                        )
                    ),
                ),
...
class HeaderLinkBlock(blocks.StructBlock):
    ...
    id = blocks.CharBlock(
        help_text=(
            "Intended to be shared with a page link button so that clicking the button"
            " will scroll the user to this header"
        )
    )
...
class ChooseViewBlock(blocks.StructBlock):
    view = blocks.ChoiceBlock(
        choices=[
            ("vs-side-by-side", "Side By Side"),
            ("vs-over-image", "Text Over Image"),
        ],
                    ...
                    [
                        (
                            ...
                            blocks.RichTextBlock(
                                ...
                                help_text=(
                                    "Write your article contents here. See"
                                    " documentation:"
                    ...
                    [
                        (
                            ...
                            video_blocks.OneOffVideoBlock(
                                ...
                                help_text=(
                                    "Use this to credit or caption videos that will"
                                    " only be associated with this current article,"
                                    " rather than entered into our video library. You"
                                    " can also embed videos in a Rich Text Block."
                                ),
                            ),
                        ),
                        ("side", ChooseSideBlock(default=("left", "Left"))),
                    ...
                    [
                        (
                            ...
                            blocks.RawHTMLBlock(
                                ...
                                help_text=(
                                    "WARNING: DO NOT use this unless you really know"
                                    " what you're doing!"
                                ),
                            ),
                        ),
...
class GalleryBlock(blocks.StructBlock):
    id = blocks.CharBlock(
        help_text=(
            "No numbers or spaces. Must be different from other ids defined in this"
            " article."
        ),
        required=True,
    )
    images = blocks.ListBlock(image_blocks.ImageBlock())
