...
from wagtail.admin.panels import (  # Panels; Custom admin tabs
    FieldPanel,
    FieldRowPanel,
    HelpPanel,
    InlinePanel,
...
class UbysseyMenuMixin(models.Model):
    ...
    menu_content_panels = [
        MultiFieldPanel(
            [
                HelpPanel(
                    "<p>If the article has a special menu, as when it belongs to a"
                    " special series of articles, select the relevant menu"
                    " here</p><p>Alternatively, tick the box and select a page to"
                    " create a menu from</p>"
                ),
                FieldPanel("menu"),
...
class MagazineArticleBylineOrderable(Orderable):
    ...
    panels = [
        MultiFieldPanel(
            ...
            heading="Byline",
            help_text=(
                "Legacy field. 'Magazine' type articles typically allowed for custom"
                " bylines, rather than using the ones ArticlePages could generate"
                " automatically. While future magazines COULD continue to use these"
                " custom bylines, this tends to create confusion and users entering"
                " lots of information that is redundant accross fields (with no formal"
                " guarantee of that redundancy, disallowing the removal of this field"
                " to recreate bylines from some single source of truth)."
            ),
        ),
    ]
...
class TimelineSnippet(models.Model):
    ...
    def update_data(self) -> None:
        ...
        if len(qs) > 0:
            list_of_dictified_articles = list(
                qs.values(
                    "id",
                    "fw_above_cut_lede",
                    "timeline_date",
            ...
            for i, dictified_article in enumerate(list_of_dictified_articles):
                try:
                    list_of_dictified_articles[i]["featured_media"] = (
                        ArticleFeaturedMediaOrderable.objects.get(
                            id=[dictified_article["featured_media"]]
                        )
                ...
                try:
                    list_of_dictified_articles[i]["timeline_date"] = dictified_article[
                        "timeline_date"
                    ].strftime("%Y-%m-%dT%H:%M:%S.%fZ")
                except:
                    list_of_dictified_articles[i][
                        "timeline_date"
                    ] = timezone.now.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
...
class ArticlePageTag(TaggedItemBase):
    ...
    content_object = ParentalKey(
        "article.ArticlePage", on_delete=models.CASCADE, related_name="tagged_items"
    )
...
class ArticlePageManager(PageManager):
    def get_queryset(self):
        ...
        """
        return super().get_queryset().prefetch_related("featured_media__image")
    ...
    def from_magazine_special_section(
        self, section_slug="", section_root=None
    ) -> QuerySet:
        ...
        if section_slug:
            try:
                section_root = SpecialLandingPage.objects.get(
                    category__slug=section_slug
                )
                articles = (
                    self.live()
                    .public()
...
class ArticlePage(RoutablePageMixin, SectionablePage, UbysseyMenuMixin):
    ...
    explicit_published_at = models.DateTimeField(
        ...
        help_text=(
            "Techically optional (computer will fill it in for you if you do not)."
            " Publication date which is explicitly shown to the reader. Articles are"
            " seperately date/timestamped for database use; if this field is left"
            ' blank, it will by default be set to the "first published date" on'
            " publication."
        ),
    )
    ...
    show_last_modified = models.BooleanField(
        default=False,
        help_text=(
            "Check this to alert readers the article has been revised since its"
            " publication."
        ),
    )
    ...
    tags = ClusterTaggableManager(
        ...
        related_name="tags",
        help_text=(
            "ADD 'Top stories' IF YOU WANT IT TO GO ON TOP STORIES LIST. Tags entered"
            " here will be listed in the tag page with the format"
            " 'https://ubyssey.ca/tag/top-stories/'."
        ),
        verbose_name="Tags",
    )
    primary_tag_slug = models.CharField(
        ...
        max_length=255,
        help_text=(
            "IF USED, SHOULD ALSO BE IN TAGS FIELD. Enter the slug of the tag to be"
            " used for linking at the end of the article. For example, the slug of the"
            " tag 'blue chip' is 'blue-chip'."
        ),
    )
    tag_page_link = models.BooleanField(
        ...
        default=False,
        help_text=(
            "Check this if you want to add a link to the primary tag page at the end of"
            " the article."
        ),
    ...
    filter_by_tags = models.BooleanField(
        ...
        default=False,
        help_text=(
            "CHECK THIS to fill the suggested bar with other articles in this section"
            " that are also tagged with the primary tag."
        ),
    ...
    noindex = models.BooleanField(
        ...
        help_text=(
            "Warning: Only to be used when an article is requested to be unpublished,"
            " as per unpublishing policy. Should be FALSE in all but exceptional"
            " circumstances!"
        ),
    )
    # -----Setting panel stuff-----
    is_explicit = models.BooleanField(
        ...
        help_text=(
            "Check if this article contains advertiser-unfriendly content. Disables ads"
    ...
    layout = models.CharField(
        ...
        help_text=(
            'These correspond to very frequently used templates. More "bespoke",'
            " one-off templates should be added to the library of DB Templates"
        ),
        max_length=100,
    )
    ...
    fw_alternate_title = models.CharField(
        ...
        help_text=(
            'When there is a "special feature" or full-width style article, sometimes'
            " we would like to override the title as it render in the template"
        ),
        max_length=255,
    )
    ...
    title_tag = models.CharField(
        ...
        help_text=(
            "This appears above the title. It mimics the title tags in the print issue."
        ),
        max_length=255,
    )
    ...
    fw_above_cut_lede = models.TextField(
        ...
        help_text=(
            "Articles that use a special header/banner often contain a second"
            " lede/abstract summary "
        ),
    )
    ...
    show_timeline = models.BooleanField(
        default=False,
        help_text=(
            "Layout MUST be full-width (or else customized) to display a timeline"
        ),
    )
        ...
        + [
            ...
            MultiFieldPanel(
                [
                    HelpPanel(
                        content=(
                            "<h1>Help: Writing Articles</h1><p>The main contents of the"
                            ' article are organized into "blocks". Click the + to add a'
                            " block. Most article text should be written in Rich Text"
                            " Blocks, but many other features are"
                            " available!</p><p>Blocks simply represent units of the"
                            " article you may wish to re-arrange. You do not have to"
                            " put every individual paragraph in its own block (doing so"
                            " is probably time consuming!). Many articles that have"
                            " been imported into our database DO divide every paragraph"
                            " into its own block, but this is for computer convenience"
                            " during the import.</p>"
                        )
                    ),
                    FieldPanel("content"),
            ...
            MultiFieldPanel(
                [
                    HelpPanel(
                        content=(
                            'Authors may be created by creating an "Author Page", then'
                            " selected here."
                        )
                    ),
                    InlinePanel(
                        "article_authors", min_num=1, max_num=20, label="Author"
                    ),
                ],
                heading="Author(s)",
                classname="collapsible",
    ...
    promote_panels = Page.promote_panels + [
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        '"Breaking Timeout" is irrelevant if news is not breaking news.'
                    )
                ),
        ...
        MultiFieldPanel(
            ...
            help_text=(
                'In Dispatch, "SEO Keyword" was referred to as "Focus Keywords", and '
                ' "SEO Description" was referred to as "Meta Description"'
            ),
        ),
        MultiFieldPanel(
            [
        ...
        MultiFieldPanel(
            [
                FieldPanel(
                    "is_explicit",
                    help_text=(
                        "Check if this article contains advertiser-unfriendly content."
                        " Disables ads for this specific article."
                    ),
                ),
            ],
    ...
    fw_article_panels = [
        HelpPanel(
            content=(
                "<h1>Help</h1><p>IF you need an alternate layout for your article, but"
                " still a frequently-used layout (such as including a full-width"
                " banner), THEN, rather making than a highly customized frontend (as"
                " you can do in the next tab over), select the options you require"
                " here.</p> <p>The majority of articles will just use the default"
                " layout. Thus, <u>for the majority of articles, nothing on this tab"
                " should be touched</u>; the majority of these fields are not even used"
                " in most layouts. They primarily exist to keep our data organized.</p>"
            )
        ),
        MultiFieldPanel(
            [
                    ...
                    widget=Select(
                        choices=[
                            ...
                            (
                                "guide-2020",
                                (
                                    "Guide (2020 style - currently broken, last checked"
                                    " 2022/09)"
                                ),
                            ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        "<p>This information is generally used in articles that use a"
                        " full-width banner of some sort.</p>"
                    )
                ),
                FieldPanel(
                    "header_layout",
                    ...
                    help_text=(
                        'This field is used to set variations on the "Full-Width Story"'
                        " and similar layouts."
                    ),
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        "<h1>Warning</h1><p>If a timeline is included in your article,"
                        " <u>additional processing will be required when the article is"
                        " saved</u>.</p><p>It is recommended you add a Timeline snippet"
                        " LAST, <i>after</i> your article is otherwise"
                        " written.</p><p><u>Developers</u> should note: the"
                        " Timeline/Article sync is accomplished with Django signals, to"
                        " prevent tight coupling of the two classes. Do not allow use"
                        " of signals to turn into noodle logic.</p>"
                    )
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        "<p>This information is generally used in a special article"
                        " that has additional credits beyond the normal byline.</p>"
                    )
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        "Somewhat legacy. These will not be used with the majority of"
                        " templates, but are used with how Magazines or Guides or some"
                        " special articles have traditionally been set up."
                    )
                ),
    ...
    customization_panels = [
        HelpPanel(
            content=(
                "<h1>Help</h1><p>This tab exists so that every aspect of the frontend"
                " for an individual article may be customized, down to the finest"
                " detail. There are three fundamental tools of frontend web programming"
                " - HTML, CSS and JavaScript, and here you may utilize all"
                " three.</p><p>Custom HTML templates, which use the Django templating"
                ' language, should be uploaded not as files/documents but as "Custom'
                ' HTML" in the site admin.\n\n Custom CSS or JavaScript should be'
                ' uploaded to "Documents"</p>'
            )
        ),
        MultiFieldPanel(
            [
                HelpPanel(
                    content=(
                        "<p>Making a template requires some understanding of how the"
                        " Django backend works, so that you might know variable names"
                        " etc. for the data that the template is supposed to"
                        " render.</p> <p>Because of the potential complexity of a"
                        " template, it is desirable to be able to quickly switch the"
                        ' article back to a default template. Turn on "Use default'
                        ' template" to use the stock template and turn it off to be'
                        " able to override the default with a custom template. Defaults"
                        ' to "on".</p>'
                    ),
                ),
        ...
        MultiFieldPanel(
            ...
            heading="Custom CSS",
            help_text=(
                'Please upload any custom CSS to "Documents", then select the'
                " appropriate document here.\n\nSelecting a non-CSS Document will cause"
                " errors."
            ),
            classname="collapsible collapsed",
        ),  # Custom CSS
        MultiFieldPanel(
            ...
            help_text=(
                'Please upload any custom JavaScript to "Documents", then select the'
                " appropriate document here.\n\nSelecting a non-JavaScript Document"
                " will cause errors."
            ),
            classname="collapsible collapsed",
        ),  # Custom JavaScript
            ...
            if k == "org_role" or k == "author":
                ...
                visuals.append(
                    [
                        k,
                        role_types_words[k]
                        + self.get_authors_string(links=True, authors_list=v),
                    ]
                )
    ...
    authors_split_out_visual_bylines = property(
        fget=get_authors_split_out_visual_bylines
    )
    ...
    def get_articles_by_tag(self, order="-first_published_at", max=5) -> QuerySet:
        ...
        if self.primary_tag_slug:
            articles_by_tag = (
                ArticlePage.objects.live()
                .child_of(self.get_parent())
                .filter(tags__slug=self.primary_tag_slug)
                .not_page(self)
                .order_by(order)[:max]
            )
        ...
        if not suggested:
            if self.category != None:
                ...
                if len(category_articles) > 0:
                    ...
                    suggested["link"] = (
                        self.category.section_page.url
                        + "category/"
                        + self.category.slug
                    )
...
class SpecialArticleLikePage(ArticlePage):
    ...
    content_panels = ArticlePage.content_panels + [
        MultiFieldPanel(
            [HelpPanel(content=""), FieldPanel("right_column_content")],
    ...
    edit_handler = TabbedInterface(
        [
            ...
            ObjectList(
                ArticlePage.fw_article_panels, heading="Layout (Stock Templates)"
            ),
            ObjectList(
                ArticlePage.customization_panels, heading="Custom Frontend (Advanced!)"
            ),
        ],
    )  # edit_handler
