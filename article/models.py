...
class UbysseyMenuMixin(models.Model):
    ...
    menu_content_panels = [
        MultiFieldPanel(
            [
                HelpPanel(
                    "<p>If the article has a special menu, as when it belongs to a special series of articles, select the relevant menu here</p><p>Alternatively, tick the box and select a page to create a menu from</p>"
                ),
                FieldPanel("menu"),
        ...
        if len(qs) > 0:
            ...
            for i, dictified_article in enumerate(list_of_dictified_articles):
                ...
                try:
                    ...
                    list_of_dictified_articles[i]["timeline_date"] = (
...
class ArticlePage(RoutablePageMixin, SectionablePage, UbysseyMenuMixin):
    ...
    content_panels = (
        ...
        + [
            ...
            MultiFieldPanel(
                [
                    HelpPanel(
                        content='Authors may be created by creating an "Author Page", then selected here.'
                    ),
                    InlinePanel(
    ...
    promote_panels = Page.promote_panels + [
        MultiFieldPanel(
            [
                HelpPanel(
                    content='"Breaking Timeout" is irrelevant if news is not breaking news.'
                ),
    ...
    fw_article_panels = [
                ...
                FieldPanel(
                    ...
                    widget=Select(
                        choices=[
                            ...
                            (
                                "guide-2020",
                                "Guide (2020 style - currently broken, last checked 2022/09)",
                            ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content="<p>This information is generally used in articles that use a full-width banner of some sort.</p>"
                ),
                FieldPanel(
                    "header_layout",
                    widget=Select(
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content="<h1>Warning</h1><p>If a timeline is included in your article, <u>additional processing will be required when the article is saved</u>.</p><p>It is recommended you add a Timeline snippet LAST, <i>after</i> your article is otherwise written.</p><p><u>Developers</u> should note: the Timeline/Article sync is accomplished with Django signals, to prevent tight coupling of the two classes. Do not allow use of signals to turn into noodle logic.</p>"
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content="<p>This information is generally used in a special article that has additional credits beyond the normal byline.</p>"
                ),
        ...
        MultiFieldPanel(
            [
                HelpPanel(
                    content="Somewhat legacy. These will not be used with the majority of templates, but are used with how Magazines or Guides or some special articles have traditionally been set up."
                ),
