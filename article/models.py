...
class ArticlePage(RoutablePageMixin, SectionablePage, UbysseyMenuMixin):
        ...
        [
            ...
            (
                ...
                blocks.TextBlock(
                    ...
                    help_text=(
                        "DO NOT USE - Legacy block. Create a block where special"
                        " dropcap styling with be applied to the first letter and the"
                        " first letter only.\n\nThe contents of this block will be"
                        ' enclosed in a <p class="drop-cap">...</p> element, allowing'
                        " its targetting for styling.\n\nNo RichText allowed."
                    ),
                ),
            ),
            (
    ...
    fw_optional_subtitle = models.CharField(
        ...
        help_text=(
            'When there is a "special feature" or full-width style article, sometime we'
            " want to add a subtitle alongside the title"
        ),
        max_length=255,
    )
    ...
    def get_authors_with_roles(self) -> str:
        ...
        for author in authors:
            if author == "author":
                authors_with_roles += "Written by " + self.get_authors_string(
                    links=True, authors_list=authors["author"]
                )
            if author == "photographer":
                string_photos += "Photos by " + self.get_authors_string(
                    links=True, authors_list=authors["photographer"]
                )
            if author == "illustrator":
                string_author += "Illustrations by " + self.get_authors_string(
                    links=True, authors_list=authors["illustrator"]
                )
            if author == "videographer":
                string_videos += "Videos by " + self.get_authors_string(
                    links=True, authors_list=authors["videographer"]
                )
        if string_written != "":
    ...
    def get_suggested(self, number_suggested=6):
        ...
        if self.category == None or len(category_articles) == 0:
            ...
            suggested = {}
            suggested["title"] = (
                "From " + self.get_parent().title + " - " + self.category.title
            )
...
class SpecialArticleLikePage(ArticlePage):
    ...
    content_panels = ArticlePage.content_panels + [
        MultiFieldPanel(
            [HelpPanel(content=""), StreamFieldPanel("right_column_content")],
