...
class LinksStreamBlock(blocks.StructBlock):
    ...
    def get_context(self, value, parent_context=None):
        ...
        events = (
            Event.objects.filter(hidden=False, end_time__gte=timezone.now())
        ...
        for i in range(len(events)):
            if events[i].start_time < today:
                ...
                if len(context["ongoing"]) + len(context["upcoming"]) > 5:
                    break
                pubdate = events[i].start_time.astimezone(
...
class ArticleGathererBlock(AbstractArticleList):
    title = blocks.CharBlock(
        required=False, max_length=255, help_text="Fill in to overwrite title"
    )
    description = blocks.TextBlock(
        required=False, help_text="Fill in to overwrite description"
    )
    section = field_block.PageChooserBlock(
        page_type="section.SectionPage", required=False
    )
    category = SnippetChooserBlock("section.CategorySnippet", required=False)
    tag_slug = field_block.CharBlock(
        help_text=(
            "Enter tag slug. For example for 'Christmas Movie' the slug would be"
            " 'christmas-movie'."
        ),
        required=False,
    )
    ...
    def get_context(self, value, parent_context=None):
        ...
        if value["section"]:
            ...
            context["articles"] = (
                ArticlePage.objects.child_of(value["section"])
        ...
        if value["category"]:
            ...
            context["link"] = (
                value["category"].section_page.url
                + "category/"
                + value["category"].slug
                + "/"
            )
...
class SpecialLandingPageBlock(AbstractArticleList):
    ...
    def get_context(self, value, parent_context=None):
        ...
        context["articles"] = [value["landing"]] + list(
            Page.objects.child_of(value["landing"]).all()
        )
        return context
...
class ManualArticles(AbstractArticleList):
    ...
    articles = blocks.ListBlock(
        field_block.PageChooserBlock(page_type="article.ArticlePage")
    )
