...
class UbysseyImage(AbstractImage):
    ...
    author = models.ForeignKey(  # main req from original
        "authors.AuthorPage", null=True, blank=True, on_delete=models.SET_NULL
    )
...
class UbysseyRendition(AbstractRendition):
    ...
    image = models.ForeignKey(
        UbysseyImage, on_delete=models.CASCADE, related_name="renditions"
    )
...
class GallerySnippet(ClusterableModel):
    ...
    legacy_pk = models.IntegerField(null=False, blank=False, default=0)
    panels = [
        MultiFieldPanel(
            [
                FieldPanel("title"),
